...
static void stationInitTaskFn(void *param) {
    ...
    task_stack_size = 0x80;
    stationSysCreateTask(
        "lightCtrler", (stationSysTaskFn_t)lightControllerTaskFn, (void *)gmon, task_stack_size,
        GMON_TASKS_PRIO_MIN, isPrivileged, &task_ptr
    );
    ...
    stationSysCreateTask(
        "netConnHandler", (stationSysTaskFn_t)stationNetConnHandlerTaskFn, (void *)gmon, task_stack_size,
        (GMON_TASKS_PRIO_MIN + 1), isPrivileged, &task_ptr
    );
    ...
    task_stack_size = 0x80;
    stationSysCreateTask(
        "DisplayHandler", (stationSysTaskFn_t)stationDisplayTaskFn, (void *)gmon, task_stack_size,
        GMON_TASKS_PRIO_MIN, isPrivileged, &task_ptr
    );
...
static void stationCreateWorkingTasks(gardenMonitor_t *gmon) {
    ...
    const unsigned char  isPrivileged = 0x1;
    stationSysCreateTask(
        "monStaInitTask", (stationSysTaskFn_t)stationInitTaskFn, (void *)gmon, task_stack_size,
        (GMON_TASKS_PRIO_MIN + 2), isPrivileged, &init_task
    );
