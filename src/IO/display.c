...
#define  GMON_PRINT_STRING_SENSOR_RECORD    (const char *)&("[Last Record]: Soil moisture: 1024, Air Temp: 134.5'C, Air Humidity: 101, Lightness: 1001.")
#define  GMON_PRINT_STRING_SENSOR_THRSHOLD  (const char *)&("[Thresold]: Soil moisture: 1001, Air Temp: 130.5'C, Lightness: 0085.")
#define  GMON_PRINT_STRING_ACTUATOR_STATUS    (const char *)&("[Actuator] Pump: PAUSE, Cooling Fan: PAUSE, Bulb: PAUSE.")
...
static uint16_t displayVerticalScroll(
    uint16_t curr_cnt, uint16_t max_cnt, gMonDisplayBlock_t *dblks, size_t len  
) {
    if(curr_cnt > max_cnt) {
...
static void displayHorizontalScroll(
    gmonPrintInfo_t *info, uint16_t scr_width, uint16_t pxl_move
) {
...
static gMonStatus staUpdatePrintStrSensorData(gmonPrintInfo_t *content, void *app_ctx)
{
    ...
    const short     fix_content_idx[] = {30, 4, 12, 5, 18, 3, 13, 4, 1, 0,};
...
static gMonStatus staUpdatePrintStrThreshold(gmonPrintInfo_t *content, void *app_ctx)
{ // this should be refreshed every time after data exchange from remote backend
    gardenMonitor_t *gmon = (gardenMonitor_t *)app_ctx;
    unsigned char   *dst_buf = NULL;
    uint8_t          num_chr = 0;

    const short     fix_content_idx[] = {27, 4, 12, 5, 15, 4, 1, 0,};
...
void  stationDisplayTaskFn(void* params)
{
    ...
    while(1) {
        ...
        for(idx = 0; idx < display_ctx->num_blocks; idx++) {
            displayHorizontalScroll(&display_ctx->blocks[idx].content, screen_width, display_ctx->config.scroll_speed);
        }
