...
static gMonStatus mqttRespToGMonResp(mqttRespStatus status_in) {
    ...
    switch (status_in) {
    case MQTT_RESP_OK:
        status_out = GMON_RESP_OK;
        break;
    case MQTT_RESP_ERRARGS:
        status_out = GMON_RESP_ERRARGS;
        break;
    case MQTT_RESP_ERRMEM:
        status_out = GMON_RESP_ERRMEM;
        break;
    case MQTT_RESP_ERR_SECURE_CONN:
        status_out = GMON_RESP_ERR_SECURE_CONN;
        break;
    case MQTT_RESP_MALFORMED_DATA:
        status_out = GMON_RESP_MALFORMED_DATA;
        break;
    case MQTT_RESP_TIMEOUT:
        status_out = GMON_RESP_TIMEOUT;
        break;
    case MQTT_RESP_ERR_EXCEED_PKT_SZ:
        status_out = GMON_RESP_ERR_MSG_ENCODE;
        break;
    ...
    case MQTT_RESP_INVALID_TOPIC:
        status_out = GMON_RESP_ERR_MSG_DECODE;
        break;
    case MQTT_RESP_ERR_CONN:
    default:
        status_out = GMON_RESP_ERR_CONN;
        break;
...
gMonStatus stationNetConnInit(gMonNet_t *net_handle) {
    ...
    gMonStatus status =
        staSetNetConnTaskInterval(net_handle, (unsigned int)GMON_CFG_NETCONN_START_INTERVAL_MS);
    if (status < 0)
        return status;
